...
var gasStations = [(distance: Int, gas: Int)]()
for _ in 0..<n {
    gasStations.append(readLine()!.split{ $0 == " " }.map{ Int($0)! }.tuple())
}
...
gasStations.sort{ ($0.0,$0.1) < ($1.0,$1.1) }

var maxDistance = P
var stopCount = 0
var passing = MinHeap<Int>()

for gasStation in gasStations {
    if maxDistance < gasStation.distance {
        while !passing.isEmpty {
            maxDistance += -passing.pop()!
            stopCount += 1
            if maxDistance >= gasStation.distance {
                break
            }
    ...
    if passing.isEmpty, maxDistance < gasStation.distance {
        stopCount = -1
        break
    } else {
        passing.insert(-gasStation.gas)
    }
